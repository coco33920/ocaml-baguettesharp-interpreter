...
let rec leven a b = 
  ...
  if min n m = 0 then max n m 
    ...
    if a.[0] = b.[0]
    then leven (minA a) (minA b)
  else 
    1 + min (min (leven (minA a) b) (leven a (minA b))) (leven (minA a) (minA b));;
...
let select_minimal_distance_word word = 
  let f = fun s -> rapideLeven s word,s in
  let l = list_of_funct in
  let l = List.map f l in
  let l = List.sort (Stdlib.compare) l in
  let _,w = List.hd l in w;;