module ArrayManipulation = struct
  ...
  let proto_access n tbl= 
    match n,tbl with 
      | Parser.Argument(Parser.I(n)),Parser.TBL arr -> if n >= (Array.length arr) then Parser.Exception "out of bound exception" else arr.(n)
      | _ -> Parser.Exception "not an array";;

  let proto_replace n tbl rp =
    match n,tbl with 
      | Parser.Argument(Parser.I(n)),Parser.TBL arr -> if n >= (Array.length arr) then Parser.Exception "out of bound exception" else let () = (arr.(n) <- rp) in Parser.Argument (Parser.Nul ())
      | _ -> Parser.Exception "not an array"
