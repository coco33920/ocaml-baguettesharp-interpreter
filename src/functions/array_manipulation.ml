  ...
  let proto_access n tbl= 
    match n,tbl with 
      | Parser.Argument(Parser.I(n)),Parser.TBL arr -> if n >= (Array.length arr) 
        then Parser.Exception (new Parser.outofbound ("The array is of length" ^ string_of_int (Array.length arr))) 
        else arr.(n)
  ...
  let proto_replace n tbl rp =
    match n,tbl with 
      | Parser.Argument(Parser.I(n)),Parser.TBL arr -> if n >= (Array.length arr)
         then Parser.Exception (new Parser.outofbound ("The array is of length" ^ string_of_int (Array.length arr)))
         else let () = (arr.(n) <- rp) in Parser.Argument (Parser.Nul ())
  ...
  let proto_create_matrix n p default = match n,p with
    | Parser.Argument(Parser.I(n)),Parser.Argument(Parser.I(p)) -> 
      (let mat = Array.make_matrix n p default in let nmap = Array.map (fun (ss) -> Parser.TBL (ss)) mat in
       Parser.TBL nmap)
    | _ -> Parser.Exception (new Parser.type_error "argument must be an integer")
  ...
  let replace list_of_arguments = 
    if List.length list_of_arguments < 3 then Parser.Exception (new Parser.arg  
      ("This function requires three arguments and you supplied " ^ string_of_int 
      (List.length list_of_arguments) ^ "arguments"))
    else let head,tail = List.hd list_of_arguments, List.tl list_of_arguments
